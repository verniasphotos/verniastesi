...
\hypersetup{
    ...
    urlcolor=cyan,
    citecolor=blue,
    pdftitle={Tesi_Verniani_Studio_Sintesi_e_Ottimizzazione},
}
...
\vspace{20mm}
\begin{center}
    {\LARGE{\bf{Progettazione e verifica di un’architettura 6G per il tracking di droni indoor assistita da RIS}}\\\vspace{5mm}}
\end{center}
\vspace{20mm}
...
\begin{minipage}[t]{0.47\textwidth}\raggedleft
	{\large{Candidato}{\normalsize\vspace{3mm}
	\bf\\ \large{Matteo Verniani\vspace{2mm}}}}
\end{minipage}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../Tesi_Architettura_6G_4K.png}
    \caption{Panoramica Generale dell'Architettura 6G con Drone assistito da RIS attive pilotato dal Controller SDN.}
...
\subsection{Validazione Spaziale NLoS: Algoritmi di Ray-Casting e attenuazione da blocchi metallici}
...
\subsection{Il Link Budget in Cascata: Analisi del rumore termico dinamico nel canale UAV-RIS-BS}
...
\subsection{Livello MAC (Fronthaul): Accesso Grant-Free per telemetria UAV massiva}
...
\subsection{Livello di Trasporto (Backhaul): Ottimizzazione tramite gRPC e Protobuf con modellazione parametrica della latenza}
...
\subsection{Logica di Controllo (O-RAN RIC): Orchestrazione delle RIS e integrazione di moduli AI/ML tramite Near-RT RIC}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../Architettura_Blocchi_Semplificata.png}
    \caption{Schema a blocchi semplificato per la separazione dei piani di elaborazione.}
...
La transizione dai modelli teorici alla validazione pratica dei paradigmi 6G richiede un ecosistema di simulazione ad alta fedeltà. In questo capitolo viene dettagliata l'architettura software e matematica del Digital Twin sviluppato, un motore computazionale custom capace di fondere la fisica della propagazione elettromagnetica con la cinematica dei velivoli a pilotaggio remoto, abilitando la prototipazione e il testing degli algoritmi di tracciamento e orchestrazione O-RAN.

\subsection{Motore Cinematico UAV: Dinamica di volo e incertezze di orientamento con l'Iniezione del Rumore di Processo (Process Noise)}
...
\subsection{Fusione Sensoriale EKF: Implementazione dell'Extended Kalman Filter, dal limite inerziale (Coasting) al reintegro dei residui (Assisted-LoS)}
...
\subsection{Controller SDN: Algoritmi K-Means per l'Ottimizzazione del Deployment (BOM), Logiche Predittive Neurali (LSTM) e Controllo Elettromagnetico}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{../SDN_Server_Architecture_Updated.png}
...
\subsubsection{gRPC Network Gateway}
\subsubsection{Tracking Engine}
\subsubsection{RIS Orchestrator}
\subsubsection{Mission Manager}
...
\begin{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{../topological_map_layout_a.png}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{../topological_map_layout_b.png}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{../topological_map_layout_c.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../Test_1_B_TopDown_Traiettoria.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../Test_1_RMSE_Temporale.png}
    \caption{Test 1 (Curva Tempo RMSE). Decadimento rapido metrico dell'errore.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../Test_1_CDF_RMSE.png}
    \caption{Test 1 (Distribuzione Cumulativa CDF). Livelli inaccettabili di inaccuratezza nell'orientamento inerziale puro.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../Test_2_B_Successo_RIS.png}
    \caption{Test 2. Con la RIS attiva per A-LoS interviene il ripristino EKF metrico.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../Test_2_RMSE_Temporale.png}
    \caption{Test 2. Andamento temporale dell'RMSE: l'Assisted-LoS corregge repentinamente la deriva spaziale e mantiene stabile il segnale.}
    \label{fig:test2_rmse_temporale}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../Test_2_CDF_RMSE.png}
    \caption{Test 2 (CDF). Valutazione dell'abbattimento della norma dell'errore (RMSE) sotto 1m.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../Test_3_Stress_Latenza.png}
    \caption{Test 3. Il netto e invalicabile perimetro strutturale d'accettazione fissato a 50 millisecondi. In oltrepasso della soglia, la divergenza geometrica dell'interferometro decade linearmente e sfocia nel panico matriciale.}
\end{figure}
...
\subsubsection{Mitigazione del Beam Misalignment: Modello Operativo "Zero-Latency-Equivalent"}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../Test_4.1_Comparativo_RMSE_Latenza.png}
    \caption{Test 4. Comparativo qualitativo RMSE/Latenza. È documentabile l'appiattimento e l'ammortamento completo del ritardo di trigger operato dall'ingegnerizzazione SDN predittiva contro lo spike disastroso della curva reattiva standardizzata.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../Test_4.2_Traiettoria_EKF_vs_LSTM.png}
...
Parallelamente allo studio asintotico di latenza stazionaria globale, è stato imperativo l'affinamento dello scrutinio inerente il \textit{run-time} cronologico istante per istante dell'unità (Figura 5.11).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../Test_4.3_RMSE_Temporale_Missione.png}
...
\subsubsection{Analisi dell'Ecosistema Elettromagnetico: Controllo SDN per la Soppressione della Blind Reflection}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../Test_4.4_SINR_vs_M_Elements.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../Test_4.5_Scatter_Errore_Overhead.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../Test_4.6_AEE_vs_Ps_Dinkelbach.png}
...
\textit{Ai nonni,}\\
che sono i miei più grandi fan. A chi mi ha detto “studia che sarai promosso” a “so che c’è la farai, perché sei forte”. A chi non c’è più, che mi guarda da là su, che non aspettavano altro di vedere questo traguardo rendendoli fieri e felici.
